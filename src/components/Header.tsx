...
export default function Header() {
  ...
  const userName = useAppSelector((RIGHTMEET) => RIGHTMEET.auth.userInfo?.name);
  const isDarkTheme = useAppSelector((RIGHTMEET) => RIGHTMEET.auth.isDarkTheme);
  ...
  const invertTheme = () => {
    const theme = localStorage.getItem("RIGHT-MEET");
    localStorage.setItem("RIGHT-MEET", theme === "light" ? "dark" : "light");
        ...
        <Link to="/">
          <EuiText>
            <h2 style={{ padding: "0 1vw" }}>
              <EuiTextColor color="#0b5cff">RIGHT MEET</EuiTextColor>
            </h2>
          </EuiText>
        </Link>,
      ],
      ...
      items: [
        <>
          {userName ? (
            <EuiText>
              <h3>
                <EuiTextColor color="white">Namsate, </EuiTextColor>
